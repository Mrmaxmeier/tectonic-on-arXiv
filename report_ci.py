...
def do_work(sample, maindoc, tectonic):
	...
	with TestEnv(sample, is_tar=(maindoc != sample.stem)) as d:
			...
			start = time.time()
			test = subprocess.run([tectonic] + ARGUMENTS + [d / maindoc], timeout=60*2, cwd=d, env=env)
			delta = time.time() - start
			results = capture_files(d, excluded=excluded)
			report = dict(sample=sample.stem, statuscode=test.returncode, seconds=delta, results=results)
